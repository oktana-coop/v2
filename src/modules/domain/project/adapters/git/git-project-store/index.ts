...
export const createAdapter = ({
  ...
  const getCurrentBranch: ProjectStore['getCurrentBranch'] = ({ projectId }) =>
    pipe(
      ensureProjectIdIsFsPath(projectId),
      Effect.flatMap((projectPath) =>
        pipe(
  ...
  const listBranches: ProjectStore['listBranches'] = ({ projectId }) =>
    pipe(
      ensureProjectIdIsFsPath(projectId),
      Effect.flatMap((projectPath) =>
        pipe(
  ...
  const deleteBranch: ProjectStore['deleteBranch'] = ({ projectId, branch }) =>
    pipe(
      ensureProjectIdIsFsPath(projectId),
      Effect.flatMap((projectPath) =>
        pipe(
  ...
  const abortMerge: ProjectStore['abortMerge'] = ({ projectId }) =>
    pipe(
      ensureProjectIdIsFsPath(projectId),
      Effect.flatMap((projectPath) =>
        pipe(
  ...
  const listProjectAssets: ProjectStore['listProjectAssets'] = (id) =>
    pipe(
      findProjectById(id),
