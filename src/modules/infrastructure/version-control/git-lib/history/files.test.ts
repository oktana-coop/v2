...
describe('getFileCommitHistory', () => {
  ...
  it('returns commits for the given file path', async () => {
    mockLog.mockResolvedValue([
      makeLogEntry('aabbccdd', 'second edit', 1735732800),
      makeLogEntry('11223344', 'first edit', 1735689600),
    ]);

    const result = await Effect.runPromise(
      getFileCommitHistory({ isoGitFs: mockFs, dir, filepath: 'doc.md' })
    );

    expect(result).toHaveLength(2);
    expect(result[0].message).toBe('second edit');
    expect(result[1].message).toBe('first edit');
    expect(mockLog).toHaveBeenCalledWith(
      expect.objectContaining({ filepath: 'doc.md' })
    );
  });

  it('returns empty array when no commits exist for the file', async () => {
    mockLog.mockRejectedValue(new IsoGitErrors.NotFoundError('not found'));

    const result = await Effect.runPromise(
      getFileCommitHistory({ isoGitFs: mockFs, dir, filepath: 'missing.md' })
    );
