...
export type GetFileCommitHistoryArgs = Omit<IsoGitDeps, 'isoGitHttp'> & {
  filepath: string;
};

export const getFileCommitHistory = ({
  isoGitFs,
  dir,
  filepath,
}: GetFileCommitHistoryArgs): Effect.Effect<Commit[], RepositoryError, never> =>
  pipe(
    Effect.tryPromise({
      try: () =>
        git.log({
